...
/**
 * HTML Article View class for the Stockinfos component
 *
 * @package		Joomla.Site
 * @subpackage	com_stockinfos
 * @since		1.5
 */
class StockinfosViewForm extends JView
{
	protected $form;
	protected $item;
	...
	public function display($tpl = null)
	{
		...
		if (empty($this->item->id)) {
			$authorised = $user->authorise('core.create', 'com_stockinfos') || (count($user->getAuthorisedCategories('com_stockinfos', 'core.create')));
		} else {
		...
		if ($this->parameters->get('enable_category') == 1) {
			...
			$category = JCategories::getInstance('Stockinfos')->get($this->parameters->get('catid', 1));
