 ...
 * @since 1.5
 */
class StockinfosModelStockinfo extends JModelItem
{
	/**
	 ...
	 * @var		string
	 */
	protected $_context = 'com_stockinfos.stockinfo';
	...
	public function getItem($pk = null)
	{
		...
		$pk = (!empty($pk)) ? $pk : (int) $this->getState('stockinfo.id');
		...
		if (!isset($this->_item[$pk])) {
			...
			try {
				...
				if (empty($data)) {
					return JError::raiseError(404,JText::_('COM_STOCKINFOS_ERROR_STOCKINFO_NOT_FOUND'));
				}
				...
				if ($user->get('guest')) {
					...
					$userId	= $user->get('id');
					$asset	= 'com_stockinfos.stockinfo.'.$data->id;
	...
	function getItemQuery($id)
	{
        ...
        /** id */
		$stockinfoId = $id;
		if ((int) ($stockinfoId) > 0) {
			$query->where('a.id = '.(int) $stockinfoId);
		}
