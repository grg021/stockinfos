...
/**
 * This models supports retrieving lists of stockinfos.
 *
 * @package		Joomla.Site
 * @subpackage	com_stockinfos
 * @since		1.6
 */
class StockinfosModelStockinfos extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':'.$this->getState('filter.stockinfo_id');
	...
	/**
	 * Get the master query for retrieving a list of stockinfos subject to the model state.
	 *
...
//echo nl2br(str_replace('#__','foo_',$query));
	...
	/**
	 * Method to get a list of stockinfos.
	 *
	...
	public function getItems()
	{
		...
		$globalParams = JComponentHelper::getParams('com_stockinfos', true);

		// Convert the parameter fields into objects.
		foreach ($items as $item)
		{
			$stockinfoParams = new JRegistry;
			$stockinfoParams->loadString($item->parameters);

			$item->layout = $stockinfoParams->get('layout');

			$item->parameters = clone $this->getState('params');
			$item->parameters->merge($stockinfoParams);

			// Compute the asset access permissions.
			// Technically guest could edit an stockinfo, but lets not check that to improve performance a little.
			if ($guest) {
            } else {
				$asset	= 'com_stockinfos.stockinfo.'.$item->id;
			...
			if ($access) {
				// If the access filter has been set, we already have only the stockinfos this user can view.
